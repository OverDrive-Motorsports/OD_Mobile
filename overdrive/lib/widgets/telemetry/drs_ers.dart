...
class _DrsErsState extends State<DrsErs> {
  ...
  Widget build(BuildContext context) {
    ...
    final drsColor = data.drs ? AppColors.green : AppColors.textMuted.withValues(alpha: 0.45);
          ...
          builder: (context, constraints) {
            ...
            return Padding(
              ...
              child: mode == TelemetryMode.small
                  ? _SmallDrs(data: data, driverId: _driverId, drsColor: drsColor, ersColor: ersColor)
                  : _LargeDrs(data: data, driverId: _driverId, drsColor: drsColor, ersColor: ersColor),
            );
          },
        ),
      ),
...
class _SmallDrs extends StatelessWidget {
  const _SmallDrs({required this.data, required this.driverId, required this.drsColor, required this.ersColor});
  ...
  Widget build(BuildContext context) {
    return Column(
      ...
      children: [
          ...
          child: Row(
            ...
            children: [
              AnimatedContainer(
                ...
                width: 7,
                height: 7,
                decoration: BoxDecoration(shape: BoxShape.circle, color: drsColor),
              ),
              const SizedBox(width: 7),
              Text(
                data.drs ? 'DRS OPEN' : 'DRS CLOSED',
                style: AppTextStyles.label(color: drsColor).copyWith(fontSize: 11, letterSpacing: 0.6),
              ),
            ],
          ),
        ),
        ...
        Row(
          children: [
            Text(data.ersMode.toUpperCase(), style: AppTextStyles.caption(color: ersColor).copyWith(fontSize: 9, fontWeight: FontWeight.w600)),
            const Spacer(),
            SizedBox(
              width: 36,
              child: Text(
                ...
                style: AppTextStyles.caption(color: ersColor).copyWith(fontSize: 9, fontWeight: FontWeight.w600),
              ),
            ),
          ],
        ),
        const SizedBox(height: 4),
        TelemetryBar(fraction: data.ersLevel, color: ersColor, trackColor: ersColor.withValues(alpha: 0.12)),
      ],
    );
  }
}
...
class _LargeDrs extends StatelessWidget {
  const _LargeDrs({required this.data, required this.driverId, required this.drsColor, required this.ersColor});
  final TelemetrySnapshot data;
  final String driverId;
  final Color drsColor;
  final Color ersColor;
        ...
        AnimatedContainer(
          ...
          child: Row(
            ...
            children: [
              ...
              Text(
                data.drs ? 'DRS OPEN' : 'DRS CLOSED',
                style: AppTextStyles.bodyBold(color: drsColor).copyWith(fontSize: 15, letterSpacing: 0.5),
              ),
            ],
          ),
        ),
        ...
        Container(
          ...
          child: Column(
            ...
            children: [
              Row(
                children: [
                  Text('ERS', style: AppTextStyles.caption(color: AppColors.textMuted).copyWith(fontSize: 9)),
                  const SizedBox(width: 6),
                  Text(data.ersMode.toUpperCase(), style: AppTextStyles.caption(color: ersColor).copyWith(fontSize: 9, fontWeight: FontWeight.w700)),
                  const Spacer(),
                  SizedBox(
                    width: 36,
                    child: Text(
                      '${(data.ersLevel * 100).toInt()}%',
                      textAlign: TextAlign.right,
                      style: AppTextStyles.bodyBold(color: ersColor).copyWith(fontSize: 14),
                    ),
                  ),
                ],
              ),
              const SizedBox(height: 6),
              TelemetryBar(fraction: data.ersLevel, color: ersColor, height: 5, trackColor: ersColor.withValues(alpha: 0.12)),
            ],
          ),
        ),
      ],
